...
- 倉庫: https://github.com/sigoden/aichat (upstream)
- Fork: LASP Final Project - Group 12
- 語言: Rust
- 平台: macOS, Linux, Windows, Android Termux

## 🆕 新增功能 (LASP Final Project)

本 fork 新增了三個主要功能,旨在提升 AIChat 的安全性、環境適應性和協作能力:

### 1. ✅ 環境自動偵測 (Auto-Setup / Environment Awareness)
**狀態:** 已完全實現

AIChat 現在會自動偵測並適應您的系統環境,確保所有 AI 生成的命令和建議都符合您的作業系統和工具鏈。

**功能特點:**
- 🖥️ **作業系統偵測**: 自動識別 macOS, Linux, Windows, WSL
- 🐚 **Shell 類型偵測**: 支援 Bash, Zsh, Fish, PowerShell, Cmd
- 📦 **套件管理器偵測**: 識別 Brew, Apt, Pacman, Nix, Choco, Scoop, Winget
- 💻 **系統資訊收集**: CPU 核心數、記憶體容量、硬碟空間、GPU 型號
- 🤖 **AI 環境感知**: 將環境資訊自動注入 AI prompt,確保建議的命令與您的系統相容

**使用範例:**
```bash
# 在 macOS 上執行
aichat -c "安裝 Python"
# AI 會建議: brew install python (而非 apt install python)

# 在 Ubuntu 上執行相同指令
aichat -c "安裝 Python"
# AI 會建議: sudo apt install python3
```

**技術實現:**
- 新增 `src/config/environments.rs` 模組
- 環境資訊以 XML 格式注入所有 role prompts
- 在 `.info` 命令中可查看偵測到的環境資訊

### 2. ✅ Session 匯出為 Markdown (Session Export)
**狀態:** 已完全實現

將 REPL 對話 session 匯出為格式化的 Markdown 文件,方便分享、存檔或提交作業。

**功能特點:**
- 📄 **完整對話記錄**: 保留所有提問和回應
- 🎨 **Markdown 格式化**: 自動格式化程式碼區塊和語法高亮
- 💾 **自訂檔名**: 支援指定輸出檔名
- 📊 **包含元資料**: 保留 session 名稱和設定資訊
...
> 寫一個排序演算法
> 解釋時間複雜度
> .export                      # 匯出為 session-my-project.md
> .export my-notes.md          # 匯出為自訂檔名
```

**實現位置:**
- 新增 `.export` REPL 命令
- 實現於 `src/repl/mod.rs` 中的 `export_session_markdown()` 函數

### 3. ✅ 安全執行模式 (Safe and Reversible Execution)
**狀態:** 已完全實現

AIChat 現在提供完整的安全執行機制,讓你可以放心使用 AI 生成的命令。

**功能特點:**
- ✅ **環境感知**: 命令自動符合你的作業系統
- ✅ **命令預覽** ('p'): 執行前查看會影響哪些檔案
- ✅ **自動備份**: 破壞性操作前自動建立檔案備份
- ✅ **Command Tutor 模式** ('t'): 逐步解釋命令結構和參數
- ✅ **備份管理**: 在 REPL 中使用 `.backup` 命令管理備份
- ✅ **一鍵復原**: 命令執行失敗時可立即還原檔案
...
Affected Files:
  1 [✓] /var/log/app.log
  2 [✓] /var/log/error.log
⚠️  DANGEROUS: This operation cannot be easily undone!
✓ Backup will be created automatically before execution.
...
Structure Breakdown:
  ...
  🚩 /var/log/*.log  - File pattern (wildcard)
...
Safety Notes:
  ⚠️  CAUTION: Wildcard in rm command - verify files first!
  💡 Consider using -i flag for interactive prompts

# 選擇 'e' (Execute) 執行:
✓ Backup created: a1b2c3d4-...
[執行命令]
...
[1] ID: a1b2c3d4-e5f6-7890-...
    Time: 2025-12-03T14:30:00+08:00
    Command: rm /var/log/*.log
    Files: 2 backed up
      - /var/log/app.log
      - /var/log/error.log

[2] ID: b2c3d4e5-f6a7-8901-...
    Time: 2025-12-03T13:15:00+08:00
...
# 還原特定備份
> .backup restore a1b2c3d4-e5f6-7890-...
✓ Restored: /var/log/app.log
✓ Restored: /var/log/error.log
✓ Backup a1b2c3d4-e5f6-7890-... restored successfully

# 刪除舊備份
> .backup delete b2c3d4e5-f6a7-8901-...
✓ Backup b2c3d4e5-f6a7-8901-... deleted

# 清理只保留最近50個備份
> .backup cleanup 50
✓ Cleaned up 15 old backups
```

**技術實現:**
- 新增模組: `src/utils/backup.rs` - 備份管理系統
- 新增模組: `src/utils/command_analyzer.rs` - 命令分析
- 新增模組: `src/utils/command_tutor.rs` - 命令教學
- 修改檔案: `src/main.rs` - 整合到 shell_execute
- 修改檔案: `src/repl/mod.rs` - 新增 `.backup` 命令
- 備份位置: `~/.aichat_backups/`
